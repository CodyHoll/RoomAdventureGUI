...
class Room(object):
	...
	def __init__(self, name, image, desc):
		...
		# be taken into inventory), and images (pictures of the room)
		self.name = name
		self.image = image
		self.desc = desc
		self.exits = {}
	...
	def __str__(self):
		...
		if self.name == "Front Entrance":
			s = "You are at the {}.\n".format(self.name)
		else:
			s = "You are in the {}.\n".format(self.name)

		# next, the description of the room
		s+=  self.desc + "\n"
...
class Game(Frame):
    ...
    def createRooms(self):
    	...
    	r1 = Room("Front Entrance", "room1.gif", "this is the desc")
    	r2 = Room("Living Room", "room2.gif", "this is the desc")
    	r3 = Room("Kitchen", "room3.gif", "this is the desc")
    	r4 = Room("Backyard", "room4.gif", "this is the desc")
        r5 = Room("Bedroom", "room5.gif", "this is the desc")
        r6 = Room("Hallway", "room6.gif", "this is the desc")
        r7 = Room("Bedroom", "room7.gif", "this is the desc")
        r8 = Room("Basement", "room8.gif", "this is the desc")
        r9 = Room("Pool Room", "room9.gif", "this is the desc")


    	# add exits to room 1
    	r1.addExit("north", r2)	# -> to the east of room 1 is room 2
		...
		# add exits to room 8
        r8.addExit("south", r6)
        r8.addExit("north", None)

		# adds exits to room 9
        r9.addExit("west", r4)
			...
			# the verb is: take
				...
				# a valid grabbable item is found
					if (noun == grabbable):
						...
						# items
						break
					if (noun == grabbable and noun == "key" and currentRoom == r1):
						# add the grabbable item to the player's
						# inventory
						Game.inventory.append(grabbable)
						# remove the grabbable item from the
						# room
						Game.currentRoom.delGrabbable(grabbable)
						# unlock room 8 which was previously locked
						r6.addExit("north", r8)
						# set the response (success)
						response = "---Item grabbed---.\n*~*This should fit the\
						 door north of the hallway*~*"
						# no need to check any more grabbable
						# items
						break
